...
using Moq;
using Xunit;
using UserAggregate = RentalPeAPI.User.Domain.Model.Aggregates.User;
...
namespace RentalPeAPI.Tests.Unit.Security
{
    public class UserSecurityAndRoleTests
    {
        ...
        {
            // Arrange
            var existingUser = new UserAggregate("Existing User", "existing@spacepulse.com", "hash", "+51999999999", "Homeowner", "");
        ...
        {
            // Arrange
            _mockUserRepo.Setup(r => r.FindByEmailAsync("unknown@spacepulse.com")).ReturnsAsync((UserAggregate?)null);
        ...
        {
            // Arrange
            _mockUserRepo.Setup(r => r.FindByEmailAsync(It.IsAny<string>())).ReturnsAsync((UserAggregate?)null);
